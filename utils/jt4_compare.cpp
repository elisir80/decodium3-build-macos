...
bool test_message (QString const& message, bool check_only)
{
  ...
  decodium::txmsg::EncodedMessage const encoded =
      decodium::txmsg::encodeJt4 (QString::fromLatin1 (input.constData (), 22), check_only);
  ...
  if (!check_only && encoded.tones.size () != 206)
    {
      std::fprintf (stderr,
...
int main (int argc, char** argv)
{
  ...
  if (failures != 0)
    {
      std::fprintf (stderr, "JT4 encoder test failed: %d errors\n", failures);
      return 1;
    }

  std::printf ("JT4 encoder test passed for %d messages\n", kMessages.size ());
  return 0;
}
