  ...
  for (i = 1; i <= bit_count * (*maxcycles); ++i)
    {
      ...
      if (ngamma >= nt)
        {
          ...
          if (np >= ntail)
          ...
          else
            {
              ...
              if (m0 > m1)
                {
                  tm[static_cast<std::size_t> (np)][0] = m0;
                  tm[static_cast<std::size_t> (np)][1] = m1;
                }
              else
                {
                  tm[static_cast<std::size_t> (np)][0] = m1;
                  tm[static_cast<std::size_t> (np)][1] = m0;
                  nstate[static_cast<std::size_t> (np)] += 1UL;
                }
            }
      ...
      for (;;)
        {
          bool noback = (np == 0);
          if (np > 0 && gamma[static_cast<std::size_t> (np - 1)] < nt)
            {
              noback = true;
            }
